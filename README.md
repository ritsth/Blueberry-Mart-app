...
│   │   ├── InventoryController.cs      # GET  /api/inventory/customer|shareholder
│   │   ├── OrdersController.cs         # POST /api/orders
│   │   ├── ReviewsController.cs        # POST /api/reviews
│   │   └── ShareholderController.cs    # GET  /api/shareholders/analytics
│   ├── Data/
│   │   ├── BlueberryMartDbContext.cs   # EF Core context + model config
│   │   └── DbInitializer.cs           # Dev seed data
│   ├── Models/
│   │   ├── Entities/                  # Branch, Inventory, Order,
│   │   │                              #   OrderItem, Review, User
│   │   └── Requests/                  # LoginRequest, PlaceOrderRequest
│   ├── appsettings.json               # Non-secret config (Issuer, Audience)
│   ├── appsettings.Development.example.json  # Template for local secrets
│   └── Program.cs                     # DI, middleware, JWT, startup
│
├── BlueberryMartApp/                   # Expo (React Native)
│   ├── src/
│   │   ├── screens/
│   │   │   ├── LoginScreen.tsx        # JWT login + role routing
│   │   │   ├── CustomerDashboard.tsx  # Shopping interface
│   │   │   └── ShareholderDashboard.tsx # Shop / Analytics tab toggle
│   │   ├── services/
...
├── Database/
│   └── Migrations/
│       ├── 01_InitSchema.sql          # Core tables + PG enum types
│       ├── 02_AddReviews.sql          # reviews table
│       └── 03_AddOrderItems.sql       # order_items table (analytics)
│
├── Tests/
│   └── BlueberryMart.Api.Tests/
│       ├── Infrastructure/
│       │   ├── BlueberryMartApiFactory.cs  # WebApplicationFactory + test DB
│       │   ├── IntegrationCollection.cs    # xUnit [Collection] definition
│       │   └── TestHelpers.cs              # Token helpers, PlaceOrderAsync
│       ├── AuthControllerTests.cs
│       ├── InventoryControllerTests.cs
│       ├── OrdersControllerTests.cs
│       ├── ReviewsControllerTests.cs
│       └── ShareholderControllerTests.cs
│
├── .gitignore
├── CLAUDE.md
└── README.md
