    ...
    IOptions<RedisOptions> redisOptions) : ControllerBase
{
    ...
    // ordered first, limited to active items in the requested catalogue (retail or bulk).
    // Out-of-stock past purchases are intentionally kept (the card offers "Notify me") so a
    // customer's history doesn't vanish just because an item is temporarily sold out.
    [Authorize(Roles = "Customer,Shareholder")]
    [HttpGet("reorder")]
    public async Task<IActionResult> Reorder([FromQuery] Guid branchId, [FromQuery] bool bulk = false, [FromQuery] int limit = 10)
    {
        ...
        var items = await (
            ...
            where o.UserId == userId && o.BranchId == branchId && o.Status != "cancelled"
                  ...
                  && inv.IsActive && inv.IsBulkOnly == bulk
